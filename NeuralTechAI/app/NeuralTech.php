<html>
<head>
	<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.0/css/bootstrap.min.css" integrity="sha384-PDle/QlgIONtM1aqA2Qemk5gPOE7wFq8+Em+G/hmo5Iq0CCmYZLv3fVRDJ4MMwEA" crossorigin="anonymous">

	<link rel="stylesheet" type="text/css" href="css/neuraltech.css">
</head>
<body>
    <h1>My name is Jeff</h1>
    <div class="navigation">
       <a href="">Home</a>
       <a href="neuraltech-contact.php">Contact</a>
       <a href="neuraltech-software.php">Software</a>
       <a href="neuraltech-examples.php">Examples</a>
	</div>


   
	
  <div class="column">
  <h1 class = "headclass" style="color: #D3D3D3">Welcome to NeuralTech AI</h1>
        <div class="banner">

            <img class = "image1" src="AIBanner.jpg" style="width: 100%"><img class = "image2"src="logo_transparent.png" alt="logo" style="width: 17%" />
        </div>
    </div>
    <h2>Machine learning</h2>
    <p>machine learning is what you need to know </p>
  </body>
</html>
